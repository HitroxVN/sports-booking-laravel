<x-admin-layout :title="'Khu sân'">
    ...
    <div x-data="{ open: false, venueId: null, venueName: '' }">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="p-4 text-gray-500 text-sm max-w-xs whitespace-normal">
                                @if($venue->reject_reason)
                                    <span title="{{ $venue->reject_reason }}" class="block truncate">{{ $venue->reject_reason }}</span>
                                @else
                                    —
                                @endif
                            </td>
                            <td class="p-4 text-center">
                                @if($venue->status === 'pending')
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <button type="button"
                                                @click="open = true; venueId = {{ $venue->id }}; venueName = '{{ addslashes($venue->name) }}'"
        ...
        <div x-show="open" x-cloak
            ...
            <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6" @click.outside="open = false">
                ...
                <form method="POST" :action="`/admin/venues/${venueId}/reject`">
                    @csrf
