...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 mb-6">
        ...
        <span>/</span>
        <a href="/search" class="hover:text-primary-600">Tìm sân</a>
        <span>/</span>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm mb-10">
        {{-- Banner & Cover Image --}}
        <div class="relative h-64 sm:h-80 w-full bg-zinc-900">
            @if($venue->cover_image)
                <img src="{{ asset('storage/' . $venue->cover_image) }}" alt="{{ $venue->name }}" class="w-full h-full object-cover opacity-90">
            @else
                <div class="w-full h-full flex flex-col items-center justify-center text-zinc-400 bg-gradient-to-r from-zinc-800 to-zinc-900">
        ...
        <div class="p-6 sm:p-8">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 pb-6 border-b border-zinc-100 dark:border-zinc-700">
                <div class="space-y-3">
                    <h1 class="text-2xl sm:text-4xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight">
                        {{ $venue->name }}
                    </h1>

                    <p class="text-sm text-zinc-600 dark:text-zinc-300 flex items-start gap-2">
                    ...
                    @if($venue->phone)
                        <p class="text-sm text-zinc-600 dark:text-zinc-300 flex items-center gap-2">
                ...
                <div class="flex sm:flex-col items-end justify-between sm:justify-start gap-4">
                    <div class="flex items-center gap-2 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 px-4 py-2 rounded-xl">
                        <div class="flex text-yellow-400">
                            @php $rating = round($venue->rating_avg ?? 0); @endphp
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-4 h-4 {{ $i <= $rating ? 'text-yellow-400' : 'text-zinc-300 dark:text-zinc-600' }}" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <span class="text-xs text-zinc-500 dark:text-zinc-400">({{ $venue->reviews->count() }} đánh giá)</span>
                    </div>
                    ...
                    @if($minPrice)
                        <div class="text-right">
                            <span class="text-xs text-zinc-400 dark:text-zinc-500 block">Mức giá thuê từ</span>
                            <span class="text-2xl font-extrabold text-primary-600 dark:text-primary-400">
                                {{ number_format($minPrice, 0, ',', '.') }}đ <span class="text-xs font-normal text-zinc-500 dark:text-zinc-400">/giờ</span>
                            </span>
                        </div>
                    @endif
                </div>
            ...
            @if($venue->description)
                <div class="mt-6">
                    <h3 class="text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider mb-2">Giới thiệu khu sân</h3>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h2 class="text-2xl font-extrabold text-zinc-900 dark:text-zinc-100">
                Danh sách sân con 
            </h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <aside class="lg:col-span-3 order-2 lg:order-1">
            <form action="{{ route('venues.show', $venue->slug) }}" method="GET" id="court-filter-form">
                <div class="card-base p-5 space-y-5 lg:sticky top-6">
                    <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-700 pb-3">
                        <h3 class="font-bold text-zinc-900 dark:text-zinc-100 text-base flex items-center gap-2">
                    ...
                    {{-- 1. Tìm tên sân --}}
                    <div>
                        <label class="label-eyebrow mb-2">
                            Tên sân
                        </label>
        ...
        <main class="lg:col-span-9 order-1 lg:order-2">
            <div class="card-base px-5 py-4 mb-6 flex flex-wrap items-center justify-between gap-2">
                <p class="text-sm text-zinc-600 dark:text-zinc-300">
                    Hiển thị <strong class="text-zinc-900 dark:text-zinc-100 font-bold text-base">{{ $courts->count() }}</strong> sân con thuộc {{ $venue->name }}
                </p>
            </div>
            ...
            @if($courts->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    @foreach($courts as $court)
                        <div class="group card-base card-hover overflow-hidden flex flex-col justify-between">
                            {{-- Ảnh sân con (tỉ lệ cố định 4:3 để đồng nhất bố cục) --}}
                            <div class="relative w-full aspect-[4/3] overflow-hidden bg-zinc-100 dark:bg-zinc-700">
                                @if($court->image)
                                    <img src="{{ asset('storage/' . $court->image) }}"
                                         alt="{{ $court->name }}"
                                         class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                                    <div class="absolute inset-0 flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-500 bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-700 dark:to-zinc-800">
                                ...
                                @else
                                    <span class="absolute top-3 right-3 bg-amber-500/90 backdrop-blur-md text-white text-xs font-bold px-2.5 py-1 rounded-full border border-white/20">
                                        {{ $court->status_name }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-start justify-between gap-2 mb-3">
                                    <div>
                                        <span class="text-xs font-semibold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/30 border border-primary-200 dark:border-primary-800 px-2.5 py-0.5 rounded-full inline-block mb-1">
                                            {{ $court->sport->name ?? 'Môn thể thao' }}
                                        </span>
                                        <h3 class="font-bold text-zinc-900 dark:text-zinc-100 text-lg leading-snug">
                                            {{ $court->name }}
                                        </h3>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-1.5 text-xs text-zinc-600 dark:text-zinc-300 mb-4 bg-zinc-50 dark:bg-zinc-700/50 p-3 rounded-xl">
                                    <div class="flex items-center justify-between">
                                        <span class="text-zinc-400 dark:text-zinc-500">Mặt sân:</span>
                                        <span class="font-semibold text-zinc-800 dark:text-zinc-200">{{ $court->surface_type_name }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-zinc-400 dark:text-zinc-500">Sức chứa:</span>
                                ...
                                <div class="pt-3 border-t border-zinc-100 dark:border-zinc-700 flex items-center justify-between">
                                    ...
                                    @endphp
                                    <div>
                                        <span class="text-xs text-zinc-400 dark:text-zinc-500 block">Giá thuê từ</span>
                                        @if($minCourtPrice)
                                            <span class="text-base font-extrabold text-primary-600 dark:text-primary-400">
                                                {{ number_format($minCourtPrice, 0, ',', '.') }}đ <span class="text-xs font-normal text-zinc-400 dark:text-zinc-500">/giờ</span>
                                            </span>
                                        @else
                                            <span class="text-xs text-zinc-400 dark:text-zinc-500 font-medium">Liên hệ</span>
                                        @endif
                                    </div>
