...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-200 dark:border-zinc-700 pb-5">
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight">
                Tìm sân thể thao
            </h1>
        ...
        @if(request()->hasAny(['q', 'sport_id', 'city', 'sort']))
            <a href="/search" class="inline-flex items-center gap-1.5 text-xs font-semibold text-red-600 dark:text-red-400 hover:text-red-700 bg-red-50 dark:bg-red-900/30 px-3 py-2 rounded-xl border border-red-200 dark:border-red-800 transition">
        ...
        <aside class="lg:col-span-3 order-2 lg:order-1">
            <form action="/search" method="GET" id="filter-form">
                <div class="card-base p-5 space-y-5 lg:sticky top-6">
                    ...
                    {{-- 1. Tìm theo từ khóa --}}
                    <div>
                        <label for="filter-q" class="label-eyebrow mb-2">
                            Từ khóa
                        </label>
                        <div class="relative">
                            <input id="filter-q" type="text" name="q" value="{{ request('q') }}"
                                   placeholder="Tên sân, địa chỉ, quận..."
                                   class="input-base pl-9 pr-3">
                            <svg class="w-4 h-4 text-zinc-400 dark:text-zinc-500 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div>
                        <label for="filter-sport" class="label-eyebrow mb-2">
                            Môn thể thao
                        </label>
                        <select id="filter-sport" name="sport_id" class="input-base">
                    ...
                    <div>
                        <label for="filter-city" class="label-eyebrow mb-2">
                             Thành phố
                        </label>
                        <select id="filter-city" name="city" class="input-base">
                    ...
                    {{-- 4. Sắp xếp --}}
                    <div>
                        <label for="filter-sort" class="label-eyebrow mb-2">
                            Sắp xếp theo
                        </label>
                        <select id="filter-sort" name="sort" class="input-base">
        ...
        <main class="lg:col-span-9 order-1 lg:order-2">
            ...
            <div class="card-base px-5 py-4 mb-6 flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-300">
                    <span class="font-bold text-zinc-900 dark:text-zinc-100 text-base">
                        {{ $venues->total() }}
                    </span>
                    <span>khu sân thể thao được tìm thấy</span>
                    @if(request('q'))
                        <span class="text-zinc-400 dark:text-zinc-600">|</span>
                        <span>Từ khóa: <strong class="text-primary-600 dark:text-primary-400">"{{ request('q') }}"</strong></span>
                    @endif
                </div>
                ...
                <div class="hidden sm:flex items-center gap-2">
                    @if(request('sport_id'))
                        ...
                        @if($selectedSport)
                            <span class="inline-flex items-center gap-1 text-xs bg-primary-50 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 px-2.5 py-1 rounded-full border border-primary-200 dark:border-primary-800">
                                {{ $selectedSport->name }}
                            </span>
                        @endif
                    @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    @foreach($venues as $venue)
                        <a href="/venues/{{ $venue->slug }}"
                           class="group card-base card-hover overflow-hidden flex flex-col">

                            {{-- Image container --}}
                            <div class="relative h-48 overflow-hidden bg-zinc-100 dark:bg-zinc-700">
                            ...
                            <div class="p-5 flex-1 flex flex-col justify-between">
                                <div>
                                    {{-- Tên khu sân --}}
                                    <h3 class="font-bold text-zinc-900 dark:text-zinc-100 text-base group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors line-clamp-1 mb-1">
                                        {{ $venue->name }}
                                    </h3>
                                    ...
                                    @if($sportNames->count() > 0)
                                        <div class="flex flex-wrap gap-1.5 mb-4">
                                            @foreach($sportNames as $sportName)
                                                <span class="text-xs font-medium bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 border border-primary-200 dark:border-primary-800 px-2 py-0.5 rounded-md">
                                                    {{ $sportName }}
                                                </span>
                                            @endforeach
                                        </div>
                                ...
                                <div class="pt-3 border-t border-zinc-100 dark:border-zinc-700 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        <div class="flex text-yellow-400">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-3.5 h-3.5 {{ $i <= $rating ? 'text-yellow-400' : 'text-zinc-300 dark:text-zinc-600' }}" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div class="text-right">
                                        @if($minPrice)
                                            <span class="text-xs text-zinc-400 dark:text-zinc-500 block">Từ</span>
            ...
            @else
                ...
                <div class="card-base">
                    <x-empty-state icon=""
                                   title="Không tìm thấy khu sân nào phù hợp"
                                   description="Rất tiếc, không tìm thấy khu sân nào khớp với các tiêu chí tìm kiếm của bạn. Hãy thử thay đổi từ khóa hoặc xóa bớt bộ lọc."
                                   actionUrl="/search"
                                   actionText="Xem tất cả các sân"/>
                </div>
            @endif
        </main>
    </div>
