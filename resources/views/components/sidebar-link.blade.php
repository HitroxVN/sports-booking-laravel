{{-- ================================================================
     SIDEBAR LINK — link menu dùng chung cho sidebar owner/admin
     Props:
        href    : đường dẫn
        active  : bool — đang ở trang này hay không
        icon    : key icon (map bên dưới) hoặc truyền trực tiếp chuỗi SVG path
================================================================= --}}
...
@php
    $icons = [
        ...
        'pin'       => '<path stroke-linecap="round" stroke-linejoin="round" d="M6.75 2.25v.084m11.5 0V2.25m-11.5 0a2.25 2.25 0 0 0-2.25 2.25v.084m13.5-2.334a2.25 2.25 0 0 1 2.25 2.25v.084m-16.5 0h16.5m-16.5 0v10.5a2.25 2.25 0 0 0 2.25 2.25h12a2.25 2.25 0 0 0 2.25-2.25V4.5m-16.5 0V4.5m16.5 0V4.5"/>',
        'ball'      => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Zm0 0c1.5 2.5 2.25 5.5 2.25 9S13.5 18.5 12 21m0-18c-1.5 2.5-2.25 5.5-2.25 9S10.5 18.5 12 21M3.6 9h16.8M3.6 15h16.8"/>',
        ...
        'chat'      => '<path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a.75.75 0 0 1-.974-.975c.34-.988.58-2.05.706-3.136C3.125 15.42 2.25 13.785 2.25 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z"/>',
    ];
@endphp

@php
    $linkClasses = 'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors group '
        . ($active
            ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300 shadow-sm ring-1 ring-primary-600/10'
            : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-zinc-100');
    $boxClasses = 'flex items-center justify-center w-8 h-8 rounded-lg shrink-0 transition-colors '
        . ($active
            ? 'bg-primary-600 text-white'
            : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400 group-hover:bg-primary-50 group-hover:text-primary-600 dark:group-hover:bg-primary-900/30 dark:group-hover:text-primary-400');
@endphp

<a href="{{ $href }}" class="{{ $linkClasses }}" @if($active) aria-current="page" @endif>
    <span class="{{ $boxClasses }}">
        @php $iconPath = $icons[$icon] ?? $icon; @endphp
        <svg class="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">{!! $iconPath !!}</svg>
    </span>
    {{ $slot }}
</a>
